...
def Message processData(Message message) {
    ...
    if (isError) {
        message.setProperty("ErrorInCreate", "true")
        if (log) {
            log.addAttachmentAsString("CreateTimesheetErrorLog", message.getBody(String), "text/xml")
        }
    } else {
        if (log) {
            log.addAttachmentAsString("CreateTimesheetWarningPayload", message.getBody(String), "text/xml")
        }
    }
